...
public class PlaMsprtrainService extends _GenericService {
	...
	public void expExcel(PlaMsprtrain plaMsprtrain, HttpServletResponse response)
		...
		tabDefineMap.put("月非统销请车及批复计划", sheetMap);

		// 在sheetMap中加入list，这是要输出的主要数据列表
		sheetMap.put("list", list);
		// 标题
		sheetMap.put("title", "月非统销请车及批复计划");
		...
		tabDefineMap.put("日非统销请车及批复计划", sheetMap1);

		// 在sheetMap中加入list，这是要输出的主要数据列表
		sheetMap1.put("list", dups);
		// 标题
		sheetMap1.put("title", "日非统销请车及批复计划");
		...
		ExcelUtilNew.writeWorkbook(response, tabDefineMap, "月非统销请车及批复计划");
