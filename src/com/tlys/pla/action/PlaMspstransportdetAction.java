...
@Namespace("/pla")
public class PlaMspstransportdetAction extends _BaseAction {
	...
	private List<PlaMspstransportdet> plaMspstransportdets;
	...
	public String cartypes() throws Exception {
		Long id = plaMspstransportdet.getId();
	...
	public String customerstations() throws Exception {
		plaMspstransportdet = plaMspstransportdetService
				.load(plaMspstransportdet.getId());
		String customerid = "";
		if (null != plaMspstransportdet) {
	...
	public String customers() throws Exception {
		...
		String corpid = "";
		if (null != plaMspstransportdet) {
			corpid = plaMspstransportdet.getCorpid().trim();
		}
		if (logger.isDebugEnabled()) {
	...
	public String transporters() {
		...
		String corpid = "";
		if (null != plaMspstransportdet) {
			customerid = plaMspstransportdet.getCustomerid().trim();
			corpid = plaMspstransportdet.getCorpid().trim();
		}
		if (logger.isDebugEnabled()) {
	...
	public String delete() throws Exception {
		Long id = plaMspstransportdet.getId();
		plaMspstransportdet = plaMspstransportdetService.load(id);
		plaMspstransportdetService.delete(plaMspstransportdet);
	...
	public String list() throws Exception {
		String pageUrl = "/pla/pla-mspstransportdet!list.action?plannoStr="+plannoStr;
		if (null == pageCtr) {
			pageCtr = new PageCtr();
			// 当有查询条件时,将当前查询条件对象存入session中，并且将索引存在pageCtr中
			// 在生成页码跳转html时，应带上此参数
			if (null != plaMspstransportdet) {
				String schObjKey = "plaMspstransportdet_"
						+ new Date().getTime();
				pageCtr.setSchObjKey(schObjKey);
				plaMspstransportdet.setCorpids(null == corpids
						|| "".equals(corpids) ? null : corpids.split(","));
				plaMspstransportdet.setSecondkinds(null == secondkinds
				...
				plaMspstransportdet
						.setCategories(null == categories
								...
								.split(","));
				setSessionAttr(schObjKey, plaMspstransportdet);
			}
		} else {
			// 当在前台点页码进入时，应同时传来schObjKey
			if (null != pageCtr.getSchObjKey()) {
				plaMspstransportdet = (PlaMspstransportdet) getSessionAttr(pageCtr
						.getSchObjKey());
			}
		}
		pageCtr.setPageUrl(pageUrl);

		plaMspstransportdetService.find(plaMspstransportdet, pageCtr);
		String planno = plaMspstransportdet.getPlanno();
		PlaMspstransport plaMspstransport = plaMspstransportService
				.load(planno);
		if (null != plaMspstransport) {
			String status = plaMspstransport.getStatus();
		...
		for (Iterator iterator = idSet.iterator(); iterator.hasNext();) {
			...
			if (null != dicProduct.getSecondkindid()
					&& dicProduct.getSecondkindid().equals(
							plaMspstransportdet.getProductsecondid())) {
	...
	public String productseconds() {
		...
		for (Iterator iterator = idSet.iterator(); iterator.hasNext();) {
			...
			if (null != custorCategoryId
					&& custorCategoryId.equals(plaMspstransportdet
	...
	public String save() throws Exception {
		...
		Class<?> clazz = plaMspstransportdet.getClass();
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (field.getName().equals(fieldName)) {
				...
				if (fieldType.equals("Double")) {
					fields[i].set(plaMspstransportdet, Double
							.valueOf(fieldValue));
				} else if (fieldType.equals("Long")) {
					fields[i]
							.set(plaMspstransportdet, Long.valueOf(fieldValue));
				} else {
					fields[i].set(plaMspstransportdet, fieldValue);
				}
				if (fieldName.equals("customerid")) {
					if (isInnerCorp == 1) {
						...
						if (null != dicSinocorp) {
							...
							plaMspstransportdet.setCustomerid(dicSinocorp
									.getCorpid());
							plaMspstransportdet
						...
						DicCustomer dicCustomer = dicCustomerService
								.load(fieldValue);
						if (null != dicCustomer) {
							plaMspstransportdet.setCustomerid(dicCustomer
									.getCustomerid());
							plaMspstransportdet
					...
					if (null == dicSinocorp) {
						...
						plaMspstransportdet.setCustomerid(dicCustomer
								.getCustomerid());
						plaMspstransportdet.setCoustomershortname(dicCustomer
								.getShortname());
					} else {
						plaMspstransportdet.setCustomerid(dicSinocorp
								.getCorpid());
						plaMspstransportdet.setCoustomershortname(dicSinocorp
								.getShortname());
					...
					DicSinocorp dicSinocorp = dicSinocorpService
							.load(fieldValue);
					if (null != dicSinocorp) {
						plaMspstransportdet.setCorpshortname(dicSinocorp
								.getShortname());
						String areaid = dicSinocorp.getAreaid();
						plaMspstransportdet.setAreaid(areaid);
						DicAreacorp dicAreacorp = dicAreacorpService
								.load(areaid);
						if (null != dicAreacorp) {
							plaMspstransportdet.setAreashortname(dicAreacorp
									.getShortname());
						}
					}
					...
					if (null != dicProduct) {
						plaMspstransportdet.setProdductid(fieldValue);
						plaMspstransportdet.setProductcategoryid(dicProduct
								.getCategoryid());
						plaMspstransportdet.setProductsecondid(dicProduct
								.getSecondkindid());
					}
				} else if (fieldName.equals("rwkindid")) {
					DicGoods dicGoods = dicGoodsService.getDicGoods(fieldValue);
					if (null != dicGoods) {
						plaMspstransportdet.setRkindname(dicGoods.getPmhz());
					}
					...
					if (null != ctlCorpreceiver) {
						plaMspstransportdet.setReceivername(ctlCorpreceiver
								.getReceivershortname());
					}
				} else if (fieldName.equals("receivername")) {
					CtlCorpreceiver ctlCorpreceiver = new CtlCorpreceiver();
					ctlCorpreceiver.setCorpid(plaMspstransportdet
							.getCustomerid());
					ctlCorpreceiver.setCorpshortname(plaMspstransportdet
					...
					ctlCorpsender.setCorpid(plaMspstransportdet.getCorpid());
					ctlCorpsender.setCorpshortname(plaMspstransportdet
							.getCorpshortname());
					...
					plaMspstransportdet.setSenderid(senderNextReq);
				}
				break;
			}
		}
		// 修改记录
		plaMspstransportdetService.update(plaMspstransportdet);
	...
	public String sendwarehouses() throws Exception {
		...
		corpsendwarehouses = plaMupstransportdetService
	...
	public String receivers() {
		...
		String corpid = "";
		if (null != plaMspstransportdet) {
			corpid = plaMspstransportdet.getCustomerid();
		}
	...
	public String expExcel() throws Exception {
		plaMspstransportdet = (PlaMspstransportdet) getSessionAttr(schobjkey);
		HttpServletResponse response = ServletActionContext.getResponse();
		plaMspstransportdetService.expExcel(plaMspstransportdet, response);
		msg = new Msg(Msg.SUCCESS);
		return MSG;
	}

	public PlaMspstransportdet getPlaMupstransportdet() {
	...
	public void setPlaMupstransportdet(PlaMspstransportdet plaMspstransportdet) {
	...
	public List<PlaMspstransportdet> getPlaMspstransportdets() {
		return plaMspstransportdets;
	}

	public void setPlaMspstransportdets(
			List<PlaMspstransportdet> plaMspstransportdets) {
		this.plaMspstransportdets = plaMspstransportdets;
	}
